...
function openDBImg(popID) {
	Shadowbox.open({
		content: parent.getPage() + "_pop" + popID + ".html",
		player: "iframe",
		width: 629,
		height: 510
	});
...
function transformText(textIn) {
	...
	while ( textIn.indexOf(linkTxtStart, iStart) != -1 ) {
	  ...
	  } else if ( urlTxt.indexOf("image") != -1 ) {
		  iPop += 1;
		  newTxt += "<a href='javascript:openDBImg(" + iPop + ");'>" + linkTxt + "</a>";
	...
	$(rtNode).children().each(function() {
		...
		if (this.nodeName == "paragraph") {
			...
			if (nodeTxt.indexOf(linkTxtStart) != -1) {
				...
				if ( nodeTxt != "" ) { txtHTML += "<p>" + nodeTxt + "</p>"; }
			}
